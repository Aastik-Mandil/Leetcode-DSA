class Solution {
    ...
    int solveT(vector<int> &prices){
        vector<vector<vector<int>>> dp(prices.size()+1, vector<vector<int>>(3, vector<int>(2, INT_MIN)));
    ...
    int solveO(vector<int> &prices){
        vector<vector<int>> dp(3, vector<int>(2, INT_MIN)), curr(3, vector<int>(2, INT_MIN));
        dp[0][0] = curr[0][0] = 0;
        for(int cnt = 1; cnt<=2; cnt++){
            dp[cnt][0] = 0;
        }
